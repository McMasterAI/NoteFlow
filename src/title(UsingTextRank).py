...
for index, phrase in enumerate(doc._.phrases):
    ...
    for token in temp_doc:
        print(token.text, token.pos_, token.tag_, token.shape_, token.is_stop)
        if (token.shape_[0] == "X"):
            title[1] += 1
...
print("\nTitle is:", top_three[0][0] + ":", top_three[1][0] + ",", top_three[2][0])
if (top_three[0][1] > 1):
    picked_confidence = top_three[0][1] - 0.8
else:
    picked_confidence = top_three[0][1]
print("Confidence:", (picked_confidence + average)/1.1)
